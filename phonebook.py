...
def view_contacts():
    table = []
    headers = ["S/N", "Name", "Phone Number", "Favorite"]

    for index, contact in enumerate(phone_book, start=1):
        table.append([index, contact["name"], contact["phone_number"], "✔" if contact["favorite"] else "✘"])
...
def update_contact(phone_number):
    for contact in phone_book:
        if contact["phone_number"] == phone_number:
            name = input(f"Enter the new name for {contact['name']}. Leave blank to keep the same: ").strip()
            phone_num = input(f"Enter the new phone number for {contact['phone_number']}. Leave blank to keep the same: ").strip()
            if name:
                contact["name"] = name
            if phone_num:
